...
<script>
    ...
    layui.config({
        ...
        table.render({
            elem: "#myTable",
            url: "${ctx}/user/findAllChild.do?parentId="+parentId
        ...
        table.on('tool(myTable)', function (e) {
            ...
            if (layEvent == 'update') {
                var urlPage = "${ctx}/perm/toEdit.do?id="+id;
                var url = "${ctx}/user/edit.do";
                openWindow(urlPage,url,"用户编辑",400,500);
            } if (e.event == 'delete') {
                var url = '${ctx}/perm/deleteById.do?id='+id;
                checkPassWordR(url);	//校验密码.
            } if(e.event == 'updateChild'){
                var urlPage = "${ctx}/perm/toEditChild.do?id="+id;
                var url = "${ctx}/user/editChild.do";
                openWindow(urlPage,url,"编辑子菜单",400,500);
            }
        });
    })

    function add() {
        var urlPage = "${ctx}/perm/toAdd.do";
        var url = '${ctx}/perm/add.do';
